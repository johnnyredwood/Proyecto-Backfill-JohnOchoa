...
def load_invoices(**kwargs):
    ...
    start_position = 1
    while True:
        query = f"SELECT * FROM Invoice WHERE TxnDate >= '{kwargs['fecha_inicio']}' AND TxnDate <= '{kwargs['fecha_fin']}' STARTPOSITION {start_position} MAXRESULTS {kwargs['page_size']}"
        headers = {"Authorization": f"Bearer {access_token}", "Accept": "application/json"}

        r = requests.get(base_url, headers=headers, params={"query": query}, timeout=30)
        r.raise_for_status()
        data = r.json().get("QueryResponse", {}).get("Invoice", [])

        if not data:
            break

        results.extend(data)
        if len(data) < kwargs['page_size']:
            break
